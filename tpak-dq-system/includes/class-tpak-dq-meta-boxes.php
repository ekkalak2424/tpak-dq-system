...
<?php print_r($import_data); ?>
    ...
    public function render_workflow_status($post) {
        ...
        <div class="tpak-workflow-status">
            ...
            <?php if ($this->can_update_status($current_role, $current_status)): ?>
    ...
    private function can_update_status($role, $current_status) {
        ...
        return isset($permissions[$role]) && in_array($current_status, $permissions[$role]);
    }
    ...
    private function render_status_buttons($role, $current_status) {
        ...
        switch ($role) {
            case 'interviewer':
                if (in_array($current_status, array('', 'rejected_by_b', 'rejected_by_c'))) {
                    $buttons['pending_a'] = 'ส่งตรวจสอบ';
                }
                break;
                
            case 'supervisor':
                if ($current_status === 'pending_a') {
                    $buttons['pending_b'] = 'ส่งต่อ Examiner';
                    $buttons['rejected_by_b'] = 'ส่งกลับแก้ไข';
                }
                break;
            ...
            case 'examiner':
                if ($current_status === 'pending_b') {
                    ...
                    $buttons['rejected_by_c'] = 'ส่งกลับแก้ไข';
                }
                break;
        ...
        foreach ($buttons as $status => $label) {
            ?>
            <button type="button" class="button tpak-update-status" 
